...
import sys
import os
import hyperparameters_mis2 as hyperparameters
...
from model_functions import naive_previous_basket, naive_freq_basket, top_prod_acc, basket_GRU, basket_GRU_MIS2, custom_BCE
#%%Load the data
...
f_name  = "MIS2" + str(hidden_dim)+"_lr"+str(learning_rate)+"_sl"+str(schedual)+".pth"
...
#%% Initiate MI model 
network = basket_GRU_MIS2(nprod,discount_mask) 
...
for i in np.arange(num_iter):
    ...
    _, hiddens = base_model(inputs,track_hiddens = True)
    q = torch.matmul(hiddens,W_ba).detach()

    pred = network(q, output_prices)
    ...
    if ((i+1) % train_batches_per_epoch == 0):
        ...
        for j in np.arange(val_batches_per_epoch):
            ...
            _, hiddens = base_model(inputs,track_hiddens = True)
            q = torch.matmul(hiddens,W_ba).detach()
            val_pred = network(q, output_prices)
